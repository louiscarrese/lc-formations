...
@elseif($field['type'] == 'dropdown')
    <my-editable-dropdown ng-model="{{$element}}.{{$fieldId}}" editing-flag="{{$editingFlag}}" source="{{$controllerName}}.{{$field['dropdownDatasource']}}" source-id="{{$field['dropdownDataId']}}" source-label="{{$field['dropdownDataLabel']}}" model-label="{{$element}}.{{$field['dropdownLabel']}}" change="{{$field['change'] or '' }}" {{$field['additionalAttributes'] or ''}}></my-editable-dropdown>
@endif
